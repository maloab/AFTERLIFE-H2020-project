...
Storage or further sludge treatment is not jet considered."));
  ...
  protected
    ...
    //fixed
    parameter Real[n] a = {0.014, 0.05, 0.04, 2.04, 8.1, 0.65, 2.5, 0.52, 0.52};
...
//CAPEX := (Caf + Cad + Cmi + Cro) * (a[7] / 84) ^ 0.66;
    ...
    Cul := 10 ^ (3.2107 + 0.7597 * log10(a[2]) + 0.0027 * log10(a[2]) ^ 2);
...
//der(1 / q4) = Rm * Vis / (PTm * A) * (Kc * q4 * Xsf * Vis / (2 * PTm * A ^ 2));
  ...
  model AD_CO2
    ...
    parameter Real f_va_aa = 2.4 "g DQO_va/g DQO_aa";
...
//  der(Ech4) = Sch4 * Hch4;
    Ew = den * Cp / 3600 * DT * 2.04 / EHS / 1000;
    El = 0;
    Ee = ERM * 2;
...
//CO2
  ...
  model AD2_CO2
    ...
    parameter Real K_S_pro = 100 "g DQO_s/m3";
    parameter Real K_I_h2_pro = 0.0035 "g DQO/m3";
    parameter Real k_m_ac = 1.9 "g DQO_s/g DQO_x day";
    parameter Real K_S_ac = 150 "g DQO_s/m3";
    parameter Real K_I_nh3 = 1.8 "g DQO/m3";
    parameter Real k_m_h2 = 85 "g DQO_s/g DQO_x day";
    parameter Real K_S_h2 = 0.007 "g DQO_s/m3";
    parameter Real k_dec_xsu = 0.02 "day^-1";
    parameter Real k_dec_xaa = 0.02 "day^-1";
    parameter Real k_dec_xfa = 0.02 "day^-1";
    ...
    parameter Real V = 8.1 "Volume reactor m3";
    parameter Real b0 = 0.25;
    parameter Real S_IN = 0.01 "g/m3";
    parameter Real Snh3 = 0 "g/m3";
    parameter Real Hh2 = 10.8 "Lower heating value MJ/m3";
...
//  Ech4 = ((Sch4*Out.Q)/16*0.00008205746*310.15) * Hch4 * 0.27778;
    Ew = den * (Cp / 3600) * DT * 8.1 / EHS / 1000;
    El = 0;
    Ee = ERM * V;
    PPump = Out.Q * Pb / (Rb * 450) * 735.499 "W";
    KPI3 = PPump / Out.Q;
...
//Energy Balance
    EC = Ew + El * 0.277778 + Ee + Epump;
    Ew = den * Cp / 3600 * DT * 0.65 / EHS / 1000;
    El = 0;
    PPump = Out.Q * Pb / (Rb * 450) * 735.499 "W";
    KPI3 = PPump / Out.Q;
    Epump = KPI3 * Out.Q;
...
//Retenido
  ...
  equation
    ...
    Wr = sum(wr[j] for j in 1:n);
    denc = 866666.87 "Densidad experimental EO";
...
//Mr / denc;
  ...
  model Colum_ionic
    ...
    parameter Real dz = 1 / (n - 1);
    parameter Real kla = 80 "check correlation";
    parameter Real P = 1;
    parameter Real T = 200;
...
//Energy Consumption
  ...
  model ELL
    ...
    parameter Real dz = 1 / (n - 1);
    parameter Real kla = 8.4 "check correlation";
    parameter Real P = 1;
    parameter Real T = 200;
...
////Chemical cost
    ...
    Ew = den * Cp / 3600 * DT * 1.9 / EHS / 1000;
    El = 0;
    Em = ERM * V;
...
//der(1 / q4) = Rm * Vis / (PTm * A) * (Kc * q4 * Xsf * Vis / (2 * PTm * A ^ 2));
  ...
  class MAIN
    ...
    parameter Real Vaf = 2.04;
    parameter Real Vad = 8.1;
    parameter Real Var = 0.65;
  ...
  //INTRODUCE COMPOSITION
    ...
    AFTERLIFE.AD_CO2 AnaerobicFermentation(V = Vaf) annotation(
      Placement(visible = true, transformation(origin = {-14, 18}, extent = {{-14, -14}, {14, 14}}, rotation = 0)));
    AFTERLIFE.SolventSink PureWater annotation(
      Placement(visible = true, transformation(origin = {96, -114}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    AFTERLIFE.AD2_CO2 AnaerobicDigestion(V = Vad) annotation(
      Placement(visible = true, transformation(origin = {79, 87}, extent = {{-15, -15}, {15, 15}}, rotation = 0)));
    ...
    AFTERLIFE.sensor_O2 sensor_O21 annotation(
      Placement(visible = true, transformation(origin = {114, 19}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
    Modelica.Blocks.Sources.Step step1(height = 1.5) annotation(
      Placement(visible = true, transformation(origin = {104, 39}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    Modelica.Blocks.Math.Feedback feedback1 annotation(
      Placement(visible = true, transformation(origin = {113, 32}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
    ...
    AFTERLIFE.blower blower1 annotation(
      Placement(visible = true, transformation(origin = {115, -20}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
    Modelica.Blocks.Sources.Constant const(k = 15) annotation(
      Placement(visible = true, transformation(origin = {87, -29}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    AFTERLIFE.PHAStorage pHAStorage1 annotation(
      Placement(visible = true, transformation(origin = {290, 6}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    AFTERLIFE.GasSink BiogasII annotation(
      Placement(visible = true, transformation(origin = {228, 86}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    AFTERLIFE.SolventSink LiquidFertiliser annotation(
      Placement(visible = true, transformation(origin = {198, 58}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    AFTERLIFE.SolventSink EsentialOils annotation(
      Placement(visible = true, transformation(origin = {-179, -41}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
    AFTERLIFE.SolventSink Phenols annotation(
      Placement(visible = true, transformation(origin = {101, -61}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
    AFTERLIFE.Caker caker1 annotation(
      Placement(visible = true, transformation(origin = {-116, -90}, extent = {{-6, -6}, {6, 6}}, rotation = 0)));
    AFTERLIFE.ExtractorColumn2 Hydrodistillation(L = Lhy) annotation(
      Placement(visible = true, transformation(origin = {-119, -65}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
    AFTERLIFE.Decan_Limo Decantador annotation(
      Placement(visible = true, transformation(origin = {-140, -54}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    ...
    AFTERLIFE.Microfilter_Saa microfilter2 annotation(
      Placement(visible = true, transformation(origin = {15, -73}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
    ...
    AFTERLIFE.mixer2 mixer21 annotation(
      Placement(visible = true, transformation(origin = {-67, -81}, extent = {{-3, -3}, {3, 3}}, rotation = 0)));
    AFTERLIFE.mixer3 mixer51 annotation(
      Placement(visible = true, transformation(origin = {-98, 88}, extent = {{-6, -6}, {6, 6}}, rotation = 0)));
    AFTERLIFE.mixer2 mixer22 annotation(
      Placement(visible = true, transformation(origin = {48, -88}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    AFTERLIFE.MicrofilterA microfilter3 annotation(
      Placement(visible = true, transformation(origin = {173, -41}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
    ...
    AFTERLIFE.Extractor extractor1 annotation(
      Placement(visible = true, transformation(origin = {216, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    AFTERLIFE.Centrifuge centrifuge1 annotation(
      Placement(visible = true, transformation(origin = {254, 2}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    AFTERLIFE.mixer2 mixer24 annotation(
      Placement(visible = true, transformation(origin = {201, -23}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
    AFTERLIFE.divider divider1 annotation(
      Placement(visible = true, transformation(origin = {38, -16}, extent = {{-6, -6}, {6, 6}}, rotation = 0)));
    AFTERLIFE.mixer2 mixer26 annotation(
      Placement(visible = true, transformation(origin = {84, 4}, extent = {{-6, -6}, {6, 6}}, rotation = 0)));
    ...
    AFTERLIFE.mixer4 mixer27 annotation(
      Placement(visible = true, transformation(origin = {-69, -123}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
    AFTERLIFE.Hydrolysis hydrolysis1 annotation(
      Placement(visible = true, transformation(origin = {-11, -103}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
    AFTERLIFE.SolventSink aminoacids annotation(
      Placement(visible = true, transformation(origin = {25, -105}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
  equation
    connect(mixer22.In1, microfilter2.Out2) annotation(
      Line(points = {{44, -87}, {44, -74.6}, {21, -74.6}}));
    connect(ResinFractionation.In, mixer22.Out) annotation(
      Line(points = {{54.52, -44.5}, {54.52, -88}, {52, -88}}));
    connect(mixer22.In2, reverseOsmosisA1.Out1) annotation(
      Line(points = {{44, -88}, {44, -95.76}, {56.28, -95.76}, {56.28, -129.76}}));
    connect(mixer27.In4, centrifuge1.Effluent) annotation(
      Line(points = {{-76, -126}, {-76, -160.72}, {254.07, -160.72}, {254.07, -3.22}}));
    connect(mixer27.In1, Microfilter.Out2) annotation(
      Line(points = {{-76, -121}, {-87, -121}, {-87, -123.55}, {-97, -123.55}}));
    connect(ultrafilterA1.In, mixer27.Out) annotation(
      Line(points = {{-52, -122}, {-52, -122.5}, {-62, -122.5}, {-62, -123}}));
    connect(mixer27.In3, caker1.Out2) annotation(
      Line(points = {{-76, -126}, {-76, -110.22}, {-94.93, -110.22}, {-94.93, -105.22}, {-97.93, -105.22}, {-97.93, -89.22}, {-110.93, -89.22}}));
    connect(mixer27.In2, Decantador.Out2) annotation(
      Line(points = {{-76, -123}, {-76, -149.08}, {-195.51, -149.08}, {-195.51, -95.58}, {-141.01, -95.58}, {-141.01, -57.58}, {-140.51, -57.58}}));
    connect(reverseOsmosisA1.In, ultrafilterA1.Out2) annotation(
      Line(points = {{40, -129.6}, {35, -129.6}, {35, -128}, {-36, -128}}));
    connect(mixer21.In2, ultrafilterA1.Out1) annotation(
      Line(points = {{-69.79, -80.82}, {-69.79, -85.32}, {-36, -85.32}, {-36, -122}}));
    connect(pHAStorage1.In, centrifuge1.Solids) annotation(
      Line(points = {{280, 4.8}, {269.25, 4.8}, {269.25, 0.8}, {258.5, 0.8}}));
    connect(centrifuge1.In, extractor1.Effluent) annotation(
      Line(points = {{254, 2}, {254, 1}, {221, 1}}));
    connect(BiogasII.In, recovery2.GasPhase) annotation(
      Line(points = {{218, 84.8}, {218, 84.3}, {147.5, 84.3}}));
    connect(LiquidFertiliser.In, recovery2.Effluent) annotation(
      Line(points = {{188, 56.8}, {188, 68.3}, {139.5, 68.3}}));
    connect(recirculation.In, microfilter3.Out2) annotation(
      Line(points = {{198, -71.2}, {198, -43.2}, {179, -43.2}}));
    connect(extractor1.In, mixer24.Out) annotation(
      Line(points = {{210.5, -0.1}, {210.5, -24.1}, {206.5, -24.1}}));
    connect(mixer24.In2, microfilter3.Out1) annotation(
      Line(points = {{196.35, -22.7}, {196.35, -38.7}, {178.35, -38.7}}));
    connect(mixer24.In1, settler1.Waste) annotation(
      Line(points = {{196, -21.25}, {196, -5.75}, {170.5, -5.75}}));
    connect(caker1.In, Microfilter.Out1) annotation(
      Line(points = {{-119.84, -93}, {-119.84, -120}, {-95.84, -120}}));
    connect(Microfilter.In1, Wastewater.Out) annotation(
      Line(points = {{-112, -119.6}, {-112, -120.6}, {-151, -120.6}}));
    connect(PureWater.In, reverseOsmosisA1.Out2) annotation(
      Line(points = {{86, -115.2}, {86, -136.2}, {56, -136.2}}));
    connect(sensor_O21.In, PHA.MeasurePort) annotation(
      Line(points = {{114, 12}, {114, 9}, {111, 9}, {111, 16}}));
    connect(blower1.AirOut, PHA.AirIn) annotation(
      Line(points = {{114.3, -13}, {114.3, -6.5}}));
    connect(const.y, PHA.T) annotation(
      Line(points = {{91.4, -29}, {91.4, -16}, {107.4, -16}}, color = {0, 0, 255}));
    connect(settler1.In, PHA.Out) annotation(
      Line(points = {{162, 5}, {162, 3}, {123, 3}}));
    connect(PHA.In, mixer26.Out) annotation(
      Line(points = {{104, 2}, {90, 2}}));
    connect(sensor_O21.So, feedback1.u2) annotation(
      Line(points = {{120.86, 19}, {120.86, 25.5}, {112.86, 25.5}, {112.86, 28}}));
    connect(step1.y, feedback1.u1) annotation(
      Line(points = {{108.4, 39}, {108.4, 32}, {109.4, 32}}, color = {0, 0, 255}));
    connect(feedback1.y, PI.u) annotation(
      Line(points = {{117.5, 32}, {113.25, 32}, {113.25, 40}, {117, 40}}));
    connect(PI.y, blower1.u) annotation(
      Line(points = {{126.4, 40}, {126.4, 7}, {122.4, 7}, {122.4, -22}}));
    connect(SolidManure.In, recovery2.Waste) annotation(
      Line(points = {{160, 44.8}, {160, 68.3}, {136.5, 68.3}}));
    connect(recovery2.In, AnaerobicDigestion.Out) annotation(
      Line(points = {{128, 79}, {112, 79}, {112, 87}, {94, 87}}));
    connect(microfilter3.In1, settler1.Effluent) annotation(
      Line(points = {{167.4, -37.92}, {167.4, -6.42}, {173.9, -6.42}}));
    connect(Phenols.In, ResinFractionation.Out2) annotation(
      Line(points = {{94, -61.84}, {73.5, -61.84}, {73.5, -39.84}, {65, -39.84}}));
    connect(mixer26.In1, Mineralmedium.Out) annotation(
      Line(points = {{78, 6.1}, {78, 25.1}, {65, 25.1}}));
    connect(mixer26.In2, divider1.Out1) annotation(
      Line(points = {{78.42, 4.36}, {78.42, -11.64}, {42.42, -11.64}}));
    connect(AnaerobicDigestion.In, mixer51.Out) annotation(
      Line(points = {{64, 87}, {64, 88}, {-92, 88}}));
    connect(BiogasI.In, recovery1.GasPhase) annotation(
      Line(points = {{32, 50.8}, {32, 23.8}, {31.5, 23.8}, {31.5, 24.05}, {35.5, 24.05}, {35.5, 20.3}}));
    connect(mixer23.In1, ResinFractionation.Out1) annotation(
      Line(points = {{-68, 21.5}, {-68, -40}, {65, -40}}, color = {0, 0, 255}));
    connect(divider1.In, recovery1.Effluent) annotation(
      Line(points = {{32.12, -16.3}, {32.12, 4.7}, {28.12, 4.7}}));
    connect(mixer51.In2, divider1.Out2) annotation(
      Line(points = {{-103.58, 88.36}, {-103.58, 81.735}, {-99.58, 81.735}, {-99.58, -17.64}, {42.42, -17.64}}));
    connect(recovery1.In, AnaerobicFermentation.Out) annotation(
      Line(points = {{16, 15}, {16, 18}, {0, 18}}));
    connect(mixer51.In1, recovery1.Waste) annotation(
      Line(points = {{-104, 90.1}, {-104, 79.4125}, {-96, 79.4125}, {-96, 4.6}, {24.5, 4.6}}));
    connect(Hydrodistillation.In, caker1.Out1) annotation(
      Line(points = {{-123.48, -68.5}, {-123.48, -89}, {-111.48, -89}}));
    connect(microfilter2.In1, ell.Out2) annotation(
      Line(points = {{9.4, -69.92}, {9.4, -52.92}, {-4.6, -52.92}}));
    connect(hydrolysis1.In, microfilter2.Out1) annotation(
      Line(points = {{-8.27, -102.93}, {-8.27, -69.93}, {7.73, -69.93}}));
    connect(mixer21.In1, Hydrodistillation.Out2) annotation(
      Line(points = {{-70, -79.95}, {-70, -63.95}, {-113, -63.95}}));
    connect(ell.In, mixer21.Out) annotation(
      Line(points = {{-20.4, -59}, {-20.4, -81}, {-64.4, -81}}));
    connect(aminoacids.In, hydrolysis1.Aminoacids) annotation(
      Line(points = {{18, -105.84}, {5.5, -105.84}, {5.5, -103.84}, {-7, -103.84}}));
    connect(mixer51.In3, hydrolysis1.Effluent) annotation(
      Line(points = {{-103.7, 86.68}, {-103.7, 80.7425}, {-111.7, 80.7425}, {-111.7, -8.32}, {-105.7, -8.32}, {-105.7, -103.32}, {-13.7, -103.32}}));
    connect(AnaerobicFermentation.In, mixer23.Out) annotation(
      Line(points = {{-28, 18}, {-28, 17}, {-48, 17}}));
    connect(Decantador.In, Hydrodistillation.Out) annotation(
      Line(points = {{-142.56, -56}, {-127.56, -56}, {-127.56, -64}, {-112.56, -64}}));
    connect(mixer23.In2, ell.Out1) annotation(
      Line(points = {{-67.3, 18.6}, {-67.3, -53.4}, {-5.3, -53.4}}, color = {0, 0, 255}));
    connect(EsentialOils.In, Decantador.Out1) annotation(
      Line(points = {{-186, -41.84}, {-155, -41.84}, {-155, -53.84}, {-136, -53.84}}));
...
//  Decantador.mass=Hydrodistillation.mg+Hydrodistillation.Out.Xs*Hydrodistillation.Out.Q;
    for j in 1:30 loop
