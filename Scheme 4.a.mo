...
//sum(w[i] * a[i] for i in 1:n);
    yc := ((Qi + Qii + Qiii) * Cw + ChC + Qiv * cmin + 1.715 * Cfloc) * 365 + 9.38 * Cresin * 2;
...
//der(1 / q4) = Rm * Vis / (PTm * A) * (Kc * q4 * Xsf * Vis / (2 * PTm * A ^ 2));
  ...
  model AD_CO2
    ...
    parameter Real f_fa_li = 1.99 "g DQO_fa/g DQO_li";
    parameter Real f_va_aa = 0.9 "g DQO_va/g DQO_aa";
    parameter Real f_bu_su = 0.9 "g DQO_bu/g DQO_su";
    parameter Real f_bu_aa = 0.9 "g DQO_bu/g DQO_aa";
    parameter Real f_pro_aa = 0.9 "g DQO_pro/g DQO_aa";
    parameter Real f_ac_su = 1.5 "g DQO_ac/g DQO_su";
    parameter Real f_ac_aa = 1.99 "g DQO_ac/g DQO_aa";
...
//der(Xaa) = Vdot_f / V * (Xaaf - Xaa) + Y_aa * R6 - R14 "Aminoacids";
//Xsu = 1000;
    Xaa = 6000;
    Xfa = 6000;
//der(Xfa) = Vdot_f / V * (Xfaf - Xfa) + Y_fa * R7 - R15 "Fatty acids";
...
//CO2
  ...
  model AD2_CO2
    ...
    parameter Real K_S_pro = 100 "g DQO_s/m3";
    parameter Real K_I_h2_pro = 0.0035 "g DQO/m3";
    parameter Real k_m_ac = 8 "g DQO_s/g DQO_x day";
    parameter Real K_S_ac = 150 "g DQO_s/m3";
    parameter Real K_I_nh3 = 1.8 "g DQO/m3";
    parameter Real k_m_h2 = 99 "g DQO_s/g DQO_x day";
    parameter Real K_S_h2 = 0.007 "g DQO_s/m3";
    parameter Real k_dec_xsu = 0.02 "day^-1";
    parameter Real k_dec_xaa = 0.02 "day^-1";
    parameter Real k_dec_xfa = 0.02 "day^-1";
...
// Particulate fraction
    ...
    der(Xch4 / 1000) = Vdot_f / V * (Xch4f - Xch4) / 1000 + Y_c4 * R8 + Y_c4 * R9 - R16 + 0.065 "Methane";
//  Xch4 = 1000;
    der(Xpro / 1000) = Vdot_f / V * (Xprof - Xpro) / 1000 + Y_pro * R10 - R17 "Propionate";
    der(Xac / 1000) = Vdot_f / V * (Xacf / 1000 - Xac / 1000) + Y_ac * R11 - R18 + 1.05 "Acetate";
//  Xac = 25000;
...
//aqueous
  ...
  model Colum_ionic
    ...
    parameter Real dz = 1 / (n - 1);
    parameter Real kla = 60 "check correlation";
    parameter Real P = 1;
    parameter Real T = 200;
...
//Solid
  ...
  model ELL
    ...
    parameter Real D = 0.15;
    parameter Real dz = 1 / (n - 1);
    parameter Real kla = 20 "check correlation";
    parameter Real P = 1;
    parameter Real T = 200;
    parameter Real S = 3.14 / 4 * 0.15 * 0.15;
...
////Chemical cost
  ...
  //INTRODUCE COMPOSITION
    ...
    AFTERLIFE.AD_CO2 AnaerobicFermentation(V = Vaf) annotation(
      Placement(visible = true, transformation(origin = {-106, 32}, extent = {{-14, -14}, {14, 14}}, rotation = 0)));
    ...
    AFTERLIFE.MicrofilterA microfilter2 annotation(
      Placement(visible = true, transformation(origin = {-65, -55}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
    ...
    AFTERLIFE.mixer4 mixer51 annotation(
      Placement(visible = true, transformation(origin = {-166, 104}, extent = {{-6, -6}, {6, 6}}, rotation = 0)));
    ...
    AFTERLIFE.UltrafilterA ultrafilter1 annotation(
      Placement(visible = true, transformation(origin = {-52, 2}, extent = {{-6, -6}, {6, 6}}, rotation = 0)));
  equation
    connect(recovery1.In, AnaerobicFermentation.Out) annotation(
      Line(points = {{-82, 25}, {-82, 32}, {-92, 32}}));
    connect(BiogasI.In, recovery1.GasPhase) annotation(
      Line(points = {{-64, 57}, {-64, 41.8}, {-62.5, 41.8}, {-62.5, 30.5}}));
    connect(mixer51.In1, recovery1.Waste) annotation(
      Line(points = {{-172, 106}, {-172, 9.6}, {-101.75, 9.6}, {-101.75, 14.5}, {-73.5, 14.5}}));
    connect(ultrafilter1.In, recovery1.Effluent) annotation(
      Line(points = {{-57, 5}, {-57, 14.5}, {-70.5, 14.5}}));
    connect(mixer51.In4, ultrafilter1.Out1) annotation(
      Line(points = {{-172, 101}, {-172, 5}, {-47, 5}}));
    connect(divider1.In, ultrafilter1.Out2) annotation(
      Line(points = {{-32, -1}, {-33.5, -1}, {-33.5, 1}, {-47, 1}}));
    connect(microfilter2.In1, ell.Out2) annotation(
      Line(points = {{-71, -52}, {-71, -34.92}, {-88.6, -34.92}}));
    connect(mixer22.In1, microfilter2.Out2) annotation(
      Line(points = {{-44, -66.6}, {-44, -57}, {-59, -57}}));
    connect(mixer51.In2, microfilter2.Out1) annotation(
      Line(points = {{-172, 104}, {-172, -52.64}, {-112.08, -52.64}, {-112.08, -52}, {-59, -52}}));
    connect(mixer51.In3, divider1.Out2) annotation(
      Line(points = {{-172, 103}, {-172, -3}, {-24, -3}}));
    connect(mixer21.In2, divider1.Out1) annotation(
      Line(points = {{4.28, 20.24}, {4.28, 3.24}, {-5.72, 3.24}, {-5.72, 0}, {-24, 0}}));
    connect(AnaerobicDigestion.In, mixer51.Out) annotation(
      Line(points = {{-20, 105}, {-20, 104}, {-160, 104}}));
    connect(AnaerobicFermentation.In, mixer23.Out) annotation(
      Line(points = {{-120, 32}, {-109, 32}, {-109, 31}, {-132, 31}}));
